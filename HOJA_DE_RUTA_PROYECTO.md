...
- [x] Sec 3A completo — resuelto 2026-09-05. Misma metodología que Sec 2, aplicada a 2,008 combinaciones reales (antes la tabla solo tenía 721, cubriendo el 36%). De 1,372 genuinamente alimento-específicas: 546 con FC asignado (97.9% de las observaciones pendientes), 68 combos (0.66%, incluye variantes menores de "Sopita Concentrada") quedan para revisión manual, 758 sin evidencia (1.5%, cola de muy baja frecuencia). La hoja `Cuest. B Sec 3A` de `data_raw_unidades_ACTUALIZADO.xlsx` fue reconstruida completa (2,008 filas, mismo formato que Sec 2: `id_variedad`, `descripcion`, `unidad_no_estandar`, `FC`, `frec_sec3` corregida, `validacion`, `nota`). Detalle: `data/eda/FC_propuesto_sec3a.csv`.
  ...
  - [ ] **"Sopita Concentrada" — propuesta armada (2026-09-08), pendiente de aprobación de Daniel/Carlos, no un bloqueador sin dirección.** Diagnóstico preciso: el pipeline mide gramos del cubito concentrado (18,087 observaciones, alta frecuencia), pero INCAP/FNDDS solo tienen la composición del caldo YA PREPARADO/diluido — aplicarla directo subestimaría sodio/energía. Propuesta: tratar como condimento concentrado alto en sodio, no como fuente de proteína (respaldo: ficha oficial Nestlé/Maggi, carne de pollo = 0.28% del producto). Falta cerrar el número exacto: (a) conseguir foto de etiqueta nutricional real de un cubito vendido en RD, o (b) derivar la composición matemáticamente desde el factor de dilución del fabricante. Documentado en `crosswalk_tablas_composicion.xlsx`, fila de "Caldo de pollo (Sopita Concentrada)", columna `notas`.
...
- [x] **`04_equivalente_adulto.R` — primera versión completa, escrita y probada 2026-09-08.** Calcula EMA por persona (tabla FAO/WHO/UNU 2004, Tablas 4.2/4.3 para 1-17 años, Schofield 1985 + PAL=1.76 para adultos — sacadas de la fuente primaria, ya que el documento de Daniel solo las cita) y EMA por hogar. Probado contra los 28,394 registros reales: 0 personas sin EMA calculado, EMA_hogar mediana 3.04 (coherente con ~3.2 personas/hogar promedio de la ENGIH). **Limitaciones documentadas explícitamente en el propio script:** (1) el módulo sociodemográfico no pregunta embarazo/lactancia — confirmado revisando el diccionario completo, no se aplica ese ajuste; (2) peso fijo por sexo (65kg/55kg), no individual; (3) menores de 1 año usan un valor provisional (600 kcal) sin verificar contra la sección 3 del informe FAO — pendiente antes de dar ese grupo por bueno. Falta conectar con el consumo diario de `03_transform.R` para completar la fórmula de ingesta aparente.
